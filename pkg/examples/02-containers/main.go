...
import (
	"amlwwalker/gas-pump/pkg/container"
	"context"
	"crypto/ecdsa"
	"errors"
	"flag"
...
$ ./02-container -wallet [..] -address [..] list 
$ ./02-container -wallet [..] -address [..] get [container-id] 
$ ./02-container -wallet [..] -address [..] create 
...
func main() {
	...
	switch flag.Arg(0) {
	case "list":
		ids, err := container.List(ctx, cli, key)
		if err != nil {
			log.Fatal(err)
		}
	...
	case "get":
		cnr, err := container.Get(ctx, cli, flag.Arg(1))
		if err != nil {
			log.Fatal(err)
		}
	...
	case "create":
		id, err := container.Create(ctx, cli, key)
		if err != nil {
			log.Fatal(err)
		}
		...
		for i := 0; i <= 30; i++ {
			...
			_, err = container.Get(ctx, cli, id.String())
			if err == nil {
				break
			}
